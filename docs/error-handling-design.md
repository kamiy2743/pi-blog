...
GET query validation error は error として early return しない。adapter だけでは component 名や props を再構築できないため、feature handler が入力値、fallback props、validation errors を含む `Page` result を返す。
...
GET query validation error は、通常表示と partial reload のどちらでも redirect back しない。検索 URL、入力欄、現在の一覧表示を維持することを優先し、feature handler が `validationErrors` props と fallback props を持つ同じ page response を返す。
...
この場合、失敗した partial prop は現在の表示を壊さない値を返す。ただし adapter は `inertiaApp.Render` で lazy prop error を受け取った後から fallback 値を推測できない。そのため、partial reload で fallback したい prop は handler 側で fallback 契約を登録する。
...
現在の検索画面は `gonertia.Props` に lazy function を渡している。
...
この error は feature handler の `Handle` が返す error ではなく、`inertiaApp.Render` の内部で発生する。

adapter の result 描画処理でも `DisplayableError` / unexpected error を捕捉する。ただし、捕捉するだけでは partial reload の fallback props は返せない。どの prop が失敗したか、どの fallback 値を返すべきかを adapter が知る契約が必要。

```go
func renderResult(...) {
	err := inertiaApp.Render(...)
	if err != nil {
		respondRenderError(...)
	}
}
...
lazy props の error は、通常 GET page 表示中なら基本的に `ErrorPage` にする。

GET partial reload 中の lazy props error は、fallback 契約がある場合だけ `flash.error` と fallback prop として返す。adapter は `X-Inertia-Partial-Data` などの header で partial reload かを判定し、通常 GET と partial reload の response 方針を分ける。

fallback 契約は以下のように登録する。

```go
type PartialProp struct {
	Name     string
	Load     func(context.Context) (any, error)
	Fallback func(*http.Request, error) (any, *handlererror.DisplayableError)
}
```

または helper で `gonertia.Props` を組み立てる。
...
adapter は partial reload 対象の prop が `DisplayableError` を返した場合、`fallback` の値をその prop に入れ、`flash.error` に displayable message を入れて response を返す。fallback 契約がない lazy prop error は通常 GET と同じく `ErrorPage` にする。
...
GET query validation error では validation error を `error` として early return しない。adapter は component 名や fallback props を知らないため、同じ page response を再構築できない。feature handler が `Page` result を返し、その中に validation errors と fallback props を含める。
...
- GET partial reload の repository error が fallback 契約のある prop では fallback value と `flash.error` を props に返す。
- GET partial reload の repository error が fallback 契約のない prop では `ErrorPage` を返す。
